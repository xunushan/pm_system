...
> 状态：accepted
> 更新：2026-07-10
> 定位：本文档是系统架构（分层 + 组件 + 集成约束）的规范归宿，单一写入，他处引用不复述
> 决策溯源：见 doc/07（D3/D9/D13/D14/D22/D23/D24/D25/D26/D27/D28 影响本文档）
...
| 生成飞书卡片 | ✅ 讨论态卡片由子 Skill 触发推送 | ✅ 构建并发送（build_*_card + send_card，模板填充无 LLM，详见 D25）|
| 飞书卡片回调接收 | ❌ | ✅ Service `/webhook/feishu/card`，解析 `action_id` 硬编码路由 + 点击后 update_card 刷新（D25）|
...
> 约束分两区：**稳定铁律**（行为准则，少动，见 CLAUDE.md §三铁律同源）+ **易变集成**（飞书/opencode 外部行为，spike 驱动，行为指向 doc/09 / doc/07）。
> 数据模型决策（8.19/8.20/8.21 等）归 doc/07（D13/D14/D22/D23），此处只留指针。

## 8.A 稳定铁律（行为准则）

### 8.A.1 飞书回调 3 秒超时
所有确认类 API 仅执行数据库写 + 即时级联（纯 DB，<200ms），立即返回。耗时操作异步。〔同 CLAUDE.md 铁律 2〕

### 8.A.2 数据库事务与 IO 分离
事务内禁止 IO/HTTP；模式：写 DB + 即时级联 -> 提交 -> 异步 IO/HTTP。〔同 CLAUDE.md 铁律 1〕

### 8.A.3 即时级联在事务内
级联检查为纯 DB，与状态变更同事务，满足 3 秒回调。副作用异步。回退同样触发即时重算级联。〔同 CLAUDE.md 铁律 1〕

### 8.A.4 状态机约束
- 阶段：未开始/进行中/已完成/已暂停；支持进行中↔已暂停、已完成->进行中（回退，必填 reason）
- 任务：待执行/已完成/已暂停；支持待执行↔已暂停、已完成->待执行（回退，必填 reason）
- 回退/暂停写 status_change_log，回退触发即时重算级联。恢复不填 reason。〔同 CLAUDE.md 铁律 3，状态机定义见 doc/02〕
...
### 8.A.8 daily.md / weekly.md 语义
"用户确认版快照"，确认时写入。日终总结不再执行级联（级联已即时完成），仅写快照 + 标记 is_confirmed。

## 8.B 易变集成（飞书 / opencode，spike 驱动）

> 行为细节见 doc/09（卡片）/ doc/07（FACT，D27 同步返卡 / D28 GET 降级 / D26 opencode session）。本区只写约束，不复述行为。

### 8.B.1 飞书卡片数据量与 drafts
- 所有卡片避免一次性传大数据，只展示概览，回调只传标识符（draft_id/task_id 等），细节在 H5 页面。
- drafts 表保留用于确认前存储规划数据（纯存储），确认按钮回调只传 draft_id，规避飞书回调约 30KB 限制。〔决策见 doc/07 D24〕

### 8.B.2 卡片刷新与同步返卡
- 首次发送记录 message_id，更新通过"更新消息"接口（PATCH /im/v1/messages/{message_id}）刷新原卡片。
- **同步返卡**（D27 FACT）：飞书 card.action.trigger 回调的 webhook 响�体可同步返回新卡片 `{"toast":{...},"card":{"type":"raw","data":<schema2.0 卡片>}}`，点击后立即刷新，无需二次 PATCH。优先用同步返卡，比回调后另发 PATCH 更快。〔FACT 见 doc/07 D27〕
- **GET 降级**（D28 FACT）：GET /im/v1/messages 查 schema2.0 卡片返回 legacy 占位，刷新态无法回查，只能人工视觉确认。〔FACT 见 doc/07 D28〕

### 8.B.3 opencode 重试与进程模型
- 验收不通过 retry_count 累加，上限 3 次。3 次不通过不改状态，调 `DELETE /session/:sessionID` 退该 task session（全局 serve 进程不退出），用户"/pm 确认完成"后重新建/复用 session 驱动后续任务。〔决策见 doc/07 D26，修正了原 8.6/8.15 的多进程设想〕
- 全局单进程 + 多 session（方案 B），不同 workspace 用不同 session_id，全局 serve 保留服务所有 workspace。

### 8.B.4 工作空间初始化（managed 分支）
- managed=1：事务外异步初始化（mkdir+git init+骨架含规范文件）
- managed=0：校验 path 存在性，不创建任何文件，直接置已就绪
- 激活后不能改 managed。〔决策见 doc/07 D7〕

### 8.B.5 超时监控兜底
Redis KeyExpirationEvent 超时兜底。OpenCode 2 小时内完成则删 Key，未完成则触发告警。

### 8.B.6 H5 页面一致性
- DB 为唯一真相源，H5 页面调 Service API 落库，无反向同步。〔决策见 doc/07 D9〕
- 状态字段受限编辑（经 Service 校验状态机）。

### 8.B.7 Skill 与 Service 能力边界
仅"核心逻辑需要 LLM"的能力做 Skill（共 5 个）。纯确定性逻辑为 Service 代码。〔决策见 doc/07 D3，详见 doc/05〕
